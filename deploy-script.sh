...
[[ `aws elasticbeanstalk describe-environments --environment-names rtassignment-env | grep HealthStatus | cut -d: -f2 | sed -e 's/^[^"]*"//' -e 's/".*$//'` = "Ok" || `aws elasticbeanstalk describe-environments --environment-names rtassignment-env | grep HealthStatus | cut -d: -f2 | sed -e 's/^[^"]*"//' -e 's/".*$//'` = "Info"  ]]
